...
constexpr const char TAG[] = "SNK_SRV_I2S";

// todo: create a task for this in task handler
...
void audio_sink::start() {
    if (s_i2s_cb.chan_st != CHANNEL_STATUS_OPENED) {
        ESP_LOGE(TAG, "%s, TX channel wrong state: %d", __func__, s_i2s_cb.chan_st);
        return;
    }
...
void audio_sink::info_update() {
    ...
    if (mcc->type == ESP_A2D_MCT_SBC) {
        ...
        ESP_LOGI(TAG, "Configure audio player: 0x%x-0x%x-0x%x-0x%x-0x%x-%d-%d",
...
void audio_sink::data_output() {
    size_t item_size = 0;
    BaseType_t done = pdFALSE;

    if (s_i2s_cb.ringbuffer_mode == RINGBUFFER_MODE_DROPPING) {
        ESP_LOGW(TAG, "ringbuffer is full, drop this packet!");
    ...
    if (!done) {
        ESP_LOGW(TAG, "ringbuffer overflowed, ready to decrease data! mode changed: RINGBUFFER_MODE_DROPPING");
